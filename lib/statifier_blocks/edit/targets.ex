defmodule StatifierBlocks.Edit.Targets do
  ...
  **The residue is real.** Per-slot highlighting is a superset of
  ...
  def droppable_slots_for(%Document{} = document, %Palette{} = palette, %Block{} = block) do
    ...
    palette
    |> Assignability.valid_targets(document, block, %{})
    |> Enum.map(fn {parent_id, slot, _index} -> {parent_id, slot} end)
    |> Enum.uniq()
    |> Enum.reject(fn {parent_id, slot} ->
      MapSet.member?(excluded, parent_id) or full?(document, palette, parent_id, slot)
    end)
  end
