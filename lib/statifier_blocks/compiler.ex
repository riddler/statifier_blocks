defmodule StatifierBlocks.Compiler do
  ...
  **not** use "same SCXML" to conclude the document is unchanged.
  ...
  #
  # `declaring` is `nil` for every node but one: a composite that declares
  # `outcomes` (ADR-0002's Amendments C6 and C7) is no longer replaced by its
  # expansion, so it survives Resolve as a node of its own and carries the
  # two things its own emission needs and a `Context` may not - which member
  # of the expansion raises each declared name, and the expansion's own
  # members in order. Both are facts about an expansion the composite stands
  # for rather than about a child, which is why they ride here and not on the
  # context ADR-0004 decision 4 fixes.
  defmodule Resolved do
    @moduledoc false

    @type declaring :: %{
            outcomes: [{String.t(), [StatifierBlocks.Block.id()]}],
            members: [StatifierBlocks.Block.id()]
          }
    ...
    @type t :: %__MODULE__{
            ...
            declaring: declaring() | nil
          }

    @enforce_keys [:block, :module, :slots]
    defstruct [:block, :module, :slots, declaring: nil]
  end
  ...
  defp expand_node(palette, %Block{} = block, module) do
    case expand(block, module) do
      {:ok, members, param_map} ->
        ...
        |> then(fn
          {nodes, [], expansion} ->
            declaring_node(palette, block, module, members, param_map, nodes, expansion)

          {_nodes, findings, expansion} ->
            {:error, findings, expansion}
        end)
  ...
  # `ADR-0002`'s Amendments `C6` and `C7`. A composite that declares no
  # `outcomes` is replaced by its expansion outright, exactly as it always
  # was (`C3`). One that declares them compiles to a state of its own
  # enclosing that expansion, so it survives Resolve as a single node whose
  # children are the expansion members - under the reserved slot below - and
  # the author's children in the derived `on_<name>` slots, which are the one
  # kind of child a composite does NOT splice into a member.
  @spec declaring_node(
          Palette.t(),
          Block.t(),
          Palette.type_ref(),
        ...
        case reserved_slot_findings(block) ++ findings do
          [] ->
            node = %Resolved{
              block: block,
              module: module,
              slots: [{@expansion_slot, nodes} | slots],
              declaring: %{
                outcomes: Composite.outcome_raisers(palette, names, members, param_map),
  ...
  @spec outcome_slots(Palette.t(), Block.t(), [String.t()]) ::
          {[{Block.slot_name(), [Resolved.t()]}], [Finding.t()], expansion()}
  defp outcome_slots(palette, %Block{} = block, names) do
    names
    |> Enum.map(&Composite.outcome_slot/1)
    |> Enum.reduce({[], [], %{}}, fn name, {slots, findings, expansion} ->
    ...
    |> then(fn {slots, findings, expansion} -> {Enum.reverse(slots), findings, expansion} end)
  end
  ...
  defp emit(%Resolved{block: block, module: ref, slots: slots} = node, document_id) do
  ...
  # Every node's own type emits it, with one exception: a declaring
  # composite has no `emit/2` of its own to call - the macro's raises,
  # because until `C6` no composite reached Emit at all - and what its state
  ...
  #
  #     <state id="s_blk_CS" initial="s_blk_CS_body">
  #       <transition event="done.outcome.s_blk_CS_wait.received"
  #                   target="s_blk_CS__o_received" type="internal"/>
  #       ...the expansion...
  #       <final id="s_blk_CS__o_received">
  #         <onentry><raise event="done.outcome.s_blk_CS.received"/></onentry>
  #       </final>
  #     </state>
  #
  # One `<final>` per declared name, minted through `Context.outcome_id/2`
  # (`C6` item 2), so entering it raises the composite's own completion event
  # and an enclosing body can wire on it. One routing transition per member
  # that raises a declared name, targeting that outcome's slot child when the
  # author filled the slot and the outcome's final directly when they did not
  # (`C7` item 3, `core.subchart`'s shape); the child's own completion then
  # reaches that same final, so the final is reached either way and the
  # composite's outcome event fires after the continuation rather than
  # instead of it.
  @spec emit_declaring(Context.t(), Resolved.declaring()) ::
          {:ok, Emission.t()} | {:error, term()}
  defp emit_declaring(%Context{} = context, %{outcomes: outcomes, members: members}) do
    case routes(context, outcomes) do
      {:ok, routes} ->
        children =
          member_chain(members, routes) ++
            routing_transitions(routes) ++
            Enum.map(members, &Emission.child_ref/1) ++
            Enum.flat_map(routes, &slot_child/1) ++
            Enum.map(routes, &Emit.final(&1.final))

        {:ok, Emit.state(context.state_id, initial(members), children)}
  ...
  # One route per declared name, in declaration order: the `<final>` the name
  # compiles to, the members whose own outcome finals raise it, the slot
  # child that runs before that final (or `nil`), and where the routing
  # transitions therefore point.
  @typep route :: %{
           name: String.t(),
           final: StateId.t(),
           raisers: [Block.id()],
  ...
  @spec routes(Context.t(), [{String.t(), [Block.id()]}]) ::
          {:ok, [route()]} | {:error, {:invalid_outcome, Block.id(), String.t()}}
  defp routes(context, outcomes) do
    outcomes
    |> Enum.reduce_while({:ok, []}, fn {name, raisers}, {:ok, acc} ->
      case Context.outcome_id(context, name) do
        {:ok, final} ->
          child = context |> Context.children(Composite.outcome_slot(name)) |> List.first()
  ...
  defp member_chain(members, routes) do
    ...
    members
    |> Enum.zip(tl(members) ++ [nil])
    |> Enum.reject(fn {from, to} -> is_nil(to) or MapSet.member?(preempted, from) end)
    |> Enum.map(fn {from, to} ->
      [
        event: StateId.done_event(StateId.state_id(from)),
        target: StateId.state_id(to),
        internal: true
      ]
      |> Emit.transition()
      |> Emission.attributed_to(from)
    end)
  end

  # Stamped to the composite, not to the raising member: the raiser is a
  # block anywhere in the expansion, and `Emit.chain/2`'s "attributed to the
  # child it leaves" rule can only name a child of the block emitting. These
  # are the composite's own bytes about its own expansion (`C6` item 5).
  @spec routing_transitions([route()]) :: [Emission.t()]
  defp routing_transitions(routes) do
    Enum.flat_map(routes, fn %{name: name, raisers: raisers, target: target} ->
      Enum.map(raisers, fn raiser ->
        Emit.transition(
          event: StateId.outcome_event(StateId.state_id(raiser), name),
          target: target,
          internal: true
        )
      end)
  ...
  # `C7` item 4: the transition out of an occupied outcome slot's child is
  # attributed to **that child**, following `Emit.chain/2`'s rule and
  # `core.subchart`'s own.
  @spec slot_child(route()) :: [Emission.node_t()]
  defp slot_child(%{child: nil}), do: []

  defp slot_child(%{child: child, final: final}) do
    [
  ...
  @spec initial([Block.id()]) :: StateId.t()
  defp initial([first | _rest]), do: StateId.state_id(first)
  ...
  # outcome and the document put a child in it. That stays the whole of
  # the rule, and it now covers a composite: a composite that declares
  # `outcomes` derives one such slot per declared name (ADR-0002's `C7`),
  # so `unhandled?/2` below reads it like any other block's without a line
  # of change.
  #
  # What a CONTAINER does is still not it. `Emit.chain/2` wires a
  # container's children on `done.state.<child>`, which fires for every
  # final a child can reach and says nothing about which one; a container
  # that runs a step and then runs the next one has decided nothing about
  # how the step ended, and the author who filled in "if it fails" has.
  # One thing in this package does now select on a child's outcome, and it
  # is not a container above the block: a declaring composite selects on
  # `done.outcome.<member>.<name>` inside its own state, to reach its own
  # final for that name (`C6` item 3, `C7` item 3). That is the composite
  # deciding how *it* finished out of how its expansion finished, which is
  # the same authoring answer one step in.
  #
