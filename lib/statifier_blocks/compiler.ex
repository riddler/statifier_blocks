defmodule StatifierBlocks.Compiler do
  ...
  **not** use "same SCXML" to conclude the document is unchanged.
  ...
  # -- Stage 5a: the child-use finals (ADR-0004's amendment, C1) -------------

  # A document compiled for use as a child emits one top-level `<final>`
  # per outcome its root block declares, reached by a transition on that
  # outcome's completion event and carrying the outcome name as done data.
  # The root block's own outcome finals and the raises inside them are
  # untouched - what is added here is what turns that internal signal into
  # something a parent session can see.
  #
  ...
  # root block, in the `child_<outcome>` role, through the same
  # `Attribution.stamp/3` every block's own emission goes through - so the
  # provenance map stays total over the added bytes rather than growing a
  # hole nobody owns (decision 5).
  @spec child_use(Resolved.t(), Emission.t(), [option()]) :: {Emission.t(), [Emission.t()]}
  defp child_use(%Resolved{block: block, module: module}, emission, opts) do
    if Keyword.get(opts, :child_use, false) do
      pairs =
        module
        |> BlockType.outcome_names(block.config)
        |> Enum.flat_map(&child_use_outcome(block.id, &1))
  ...
  @spec child_use_outcome(Block.id(), String.t()) :: [{Emission.t(), Emission.t()}]
  defp child_use_outcome(root_id, outcome) do
    with {:ok, final_id} <- StateId.state_id(root_id, @child_role_prefix <> outcome),
         {:ok, transition} <-
           stamp_child_use(child_use_transition(root_id, outcome, final_id), root_id),
         {:ok, final} <- stamp_child_use(child_use_final(final_id, outcome), root_id) do
      [{transition, final}]
    else
      _refused -> []
    end
  ...
  @spec child_use_transition(Block.id(), String.t(), StateId.t()) :: Emission.t()
  defp child_use_transition(root_id, outcome, final_id) do
  ...
  @spec child_use_final(StateId.t(), String.t()) :: Emission.t()
  defp child_use_final(final_id, outcome) do
  ...
  @spec stamp_child_use(Emission.t(), Block.id()) ::
          {:ok, Emission.t()} | {:error, {:unknown_attribution, Block.id()}}
  defp stamp_child_use(emission, root_id) do
