defmodule StatifierBlocks.Compiler do
  ...
  ## Options
    ...
    * `:declare` - the **`<data>` roots the host declares for this
      ...
      This is the compile call's declaration surface and the only one:
      the document has no key for it (ADR-0001) and no block type
      declares one. Absent or `[]` emits no `<datamodel>` unless a block
      declares a root, so a document compiled without it is
      byte-identical to what it was before the option existed. Run
      creation still wins over `expr` (SCXML 5.3.2) - a run seeded with a
      value for the id starts from that value, which is the engine's
      behaviour and not this package's.
  ...
  **not** use "same SCXML" to conclude the document is unchanged.
  ...
  defp emit_stage(node, document_id, opts) do
    with {:ok, declared} <- host_roots(node, opts),
         {:ok, emission} <- emit(node, document_id),
         {:ok, {stripped, roots}} <- hoist(node, prepend(emission, declared)) do
