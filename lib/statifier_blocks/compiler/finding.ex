defmodule StatifierBlocks.Compiler.Finding do
  ...
  applies the generalized form at every stage:
    ...
    * `:package` - a bug in this package or in a host's block type, and no
      ...
      `:reserved_slot_name` and `:minted_id_collision` (the palette is the
      host's, not the author's; those two, in the table above, are a slot
      key and a block id an author typed, and renaming it fixes it), every
