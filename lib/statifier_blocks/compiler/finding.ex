defmodule StatifierBlocks.Compiler.Finding do
  ...
  | `:resolve` | `:unknown_block_type`, `:block_type_too_new`, `:migration_failed`, `:composite_expansion_failed` (ADR-0004's amendment of 2026-09-07, E1: `StatifierBlocks.Composite.expand!/2` raises on a broken declaration and on a root expansion with no single root, and decision 1 forbids this pipeline to raise), `:outcome_not_raisable` (ADR-0002's Amendment of 2026-09-12, `C2`: a composite declared an outcome name nothing in its expansion can raise), `:outcome_declaration_invalid` (ADR-0002's Amendment of 2026-09-18, `C9d`: a composite reading its outcomes from the instance's config answered a list this block cannot be given - not a list of names, a name twice, or a name whose derived outcome slot collides with a declared pass-through slot - or raised instead of answering; the package author's fault, and the two refusals a STATIC `:outcomes` list meets as its own module compiles), `:reserved_slot_name` (a slot on a DECLARING composite's own block spelled the way the compiler spells the slot it keeps that composite's expansion members under, whose children the resolved node would replace; an author's fault), `:undeclared_slot` and `:slot_arity_violated` (the SAME reason tuples the `:structure` row carries, drawn here for the one block that stage cannot see: a composite declaring no `outcomes` is replaced by its expansion outright (ADR-0002's `C3`), so Resolve is the last stage that holds it and the slots an author wrote on it; also the author's fault), `:minted_id_collision` (ADR-0002's Amendment of 2026-09-18, `C10`: a block an author placed in a composite block's pass-through slot or in a declaring composite's derived `on_<name>` slot, at any depth, carries an id that composite's expansion mints for one of its own members, which the Document stage's id-uniqueness check cannot see because the members do not exist until Resolve; also the author's fault) |
  ...
  applies the generalized form at every stage:
    ...
    * `:package` - a bug in this package or in a host's block type, and no
      ...
      `:reserved_slot_name`, `:undeclared_slot`, `:slot_arity_violated` and
      `:minted_id_collision` (the palette is the host's, not the author's;
      those four, in the table above, are about a slot key an author typed
      (`:reserved_slot_name`), the children an author placed in a slot
      (`:undeclared_slot`, `:slot_arity_violated`) and a block id an author
      typed (`:minted_id_collision`), and a document edit fixes each), every
      `:emit` finding that names no config key, and every `:chart` finding
      whose owning span carries no config key: an author cannot express
      `{:unresolved_target, id}`, because the block vocabulary has no way to
      name a state id.
