defmodule StatifierBlocks.Core.SubchartTest do
  ...
  describe "provenance (ADR-0004 decision 5)" do
    ...
    # Decision 9 attributes an attribute value to a config key only when
    # the value came out of that field verbatim, and this condition did
    # not: the block type composes it from an outcome name, so the bytes
    # belong to no value the author typed. Claiming them would make a
    # chart finding landing inside them read as the author's typo in a
    # field they cannot fix it from, and would let the sub-expression
    # span point into generated bytes.
    #
    # sabotage: passed `cond_key: "outcomes"` back to `Emit.transition/2`
    # -> the composed condition claims the author's field, the finding
    # below reads `fault: :author`, and this goes red (verified)
    test "the routing cond it composes is attributed to no config field" do
      compiled = compile!(subchart(nudge: true, park: true))

      condition = "_event.data.outcome === 'abandoned'"
      {element, _length} = :binary.match(compiled.scxml, ~s(<transition cond="#{condition}"))
      {value, _length} = :binary.match(compiled.scxml, condition)

      assert value > element

      assert {:ok, %{block_id: "blk_ELIG", config_key: nil}} =
               Provenance.owner_at(compiled.provenance, element)

      assert {:ok, %{block_id: "blk_ELIG", config_key: nil} = owner} =
               Provenance.owner_at(compiled.provenance, value)

      # What `StatifierBlocks.Compiler.Chart` builds from that owner for a
      # finding landing on those bytes: no config key, so the fault is the
      # package's and no span points into a value the author never typed.
      finding =
        Finding.new(:chart, {:unresolved_target, "s_blk_GONE"}, "no such target",
