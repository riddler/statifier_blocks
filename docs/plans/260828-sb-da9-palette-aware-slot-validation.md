...
- **Not touching `docs/adr/`.** ADR PRs are held for a separate review gate
  in the campaign. This plan cites records; it edits none.
...
test red, and the campaign forbids editing existing test files. So the first
...
**Stop and report rather than editing it**, per the campaign's shared-file
...
carrying a path, and the first-failing-stage rule. The campaign's rule is
...
- [ ] the diff to `compiler.ex` and `compiler/finding.ex` is additive apart
      ...
      reordered, nothing reformatted (the campaign's shared-file rule)
